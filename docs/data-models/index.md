...
| [reso-crm-opportunity-lifecycle-model.md](reso-crm-opportunity-lifecycle-model.md) | **RESO DD 2.0 → CRM Opportunity lifecycle** — verifies the as-built CDL mapping for the Qualification→Closing funnel, reconciles a proposed 24-resource model against reality (snake_case canonical, 4-tier extension model, App-DB Opportunity, no ShowingItinerary), and gives a per-resource modification plan (required/conditional/skip). |
