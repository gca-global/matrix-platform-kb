...
|---|---|---|
| `public.members` | Member | Roster identities + designations. Canonical `member_alternate_id` (RESO `MemberAlternateId`, added `20260615120000`) carries the **Azure AD object id** for Members provisioned from AD by `matrix-pipeline` (ADR-031) — the SSO-user ↔ `member_key` mapping attribute. Master-sourced by `mls-sync`; app provisioning is insert-only via `cdl-write` resource `members` (strict-RESO envelope). (Note: the `x_sir_designation` marker is spec-only — not materialized; see "SIR brand markers".) |
