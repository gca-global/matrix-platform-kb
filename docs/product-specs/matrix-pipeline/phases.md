...
>
> **Write-surface hardening (2026-06-05, post live demo).** A live Weeks-0-3 demo surfaced that the *reads* were wired but several *write* paths still 400'd/401'd against the canonical schema. Fixed without changing the `cdl-write` passthrough: (1) CDL migration `20260605120000_pipeline_week3_write_fixes.sql` adds **legacy-derivation triggers** on `contact_listings`/`contact_listing_notes` (so send-listings + add-note inserts resolve the legacy NOT NULL pivots from RESO keys), the **`x_sm_contact_key`** buyer-to-showing extension ([ADR-022](../../architecture/decisions/ADR-022.md)), and `showing_availability.listing_key`/`listing_id`. (2) App payloads corrected in `useShowingChain.ts` (canonical column names, `text[]` arrays, `timestamptz` composition, buyer via `x_sm_contact_key`), `PropertiesExplorer` (real `listing_key`), `useCaravans` (`useCompleteCaravanRun`: drop nonexistent `showing_agent_remarks`, unique `showing_key` per stop, set `caravan_key`). (3) Activities 401 fixed by wiring the SSO ES256 JWT (`postgrestAccessToken`) into the app-DB client (`kzvhqgpedapzqmwgikrw`, TPA per [ADR-018](../../architecture/decisions/ADR-018.md)).
>
