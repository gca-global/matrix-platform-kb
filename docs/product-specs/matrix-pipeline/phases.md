...
> Reflects the deployed `matrix-pipeline-2-0` codebase as of **2026-06-05** (refreshed after the Week-3 write-surface hardening + caravan timestamp fix, app `3582228`). Tags: **DONE** / **MOSTLY DONE** / **PARTIAL** / **BUILT-NOT-WIRED** / **NOT STARTED**. The per-week sections below carry the same status callout.
...
- **Week 4 — Transactions + Referral + Documents**: **RE-SCOPED + PARTIAL** (2026-06-08). Offer-to-closing/offer-economics **deferred** to a later stage (ADR-025) — kept clean of the datamodel (no `x_` offer columns). Week-4 is now **Referrals + Documents**: platform side **DONE** (`public.referral`/`public.document` CDL resources + `cdl-read`/`cdl-write` v7 + ADR-025); app UI (`/referrals`, referral surfacing, Documents tab, nav + page-key seed) **PENDING** (Lovable).
...
**Immediate next** — Week 2 is closed (both lanes). **Week 4 has been re-scoped (2026-06-08, ADR-025):** the offer-to-closing/offer-economics workstream is **deferred to a later stage**; Week-4 now delivers **Referrals + Documents** only. The platform side (CDL `referral`/`document` resources + `cdl-read`/`cdl-write` v7) is **DONE**; the remaining Week-4 work is the **Lovable app UI** (`/referrals` page + create + history, referral surfacing with manual outcome, Documents tab on `ContactDetail`, Referrals nav + `'referrals'` page-key seed). The only deferred Week-2 item is the Prospecting **email channel** (stretch) — blocked on a platform mailer, tracked alongside the same gap in `sso-member-roster-lint`.
...
> **Status (2026-06-08): RE-SCOPED + PARTIAL.** Transactions list/create/type + property close / back-on-market are live (`useTransactions`). **Scope decision (2026-06-08):** the **offer-to-closing / offer-economics workstream is DEFERRED to a later stage** — offer amount/currency/dates/terms, the TM↔buyer/property links, the offer lifecycle controls (counter/accept/sign/reject/withdraw/expire), the FR-TM-07 `Active Under Contract` push, FR-REF-08 automatic referral close-the-loop, and the forecast P&L block are NOT built this cycle. KB/codebase/datamodel are kept clean of it (no `x_` offer columns on `transaction_management`). See [ADR-025](../../architecture/decisions/ADR-025.md).
>
> Week-4 is therefore **Referrals (FR-REF) + Documents (FR-DOC)** only.
> - **Platform (Cursor) — DONE (2026-06-08):** `public.referral` + `public.document` project-flavour CDL resources (migration `20260608120000_pipeline_referral_document_tables.sql`; no RESO equivalent, no `x_`); registered in `cdl-read`/`cdl-write` (v7); ADR-025 + `cdl-schema.md`/`platform-extensions.md` updated. Authorization = `role_configurations` page-keys + CDL JWT-scope (no `sso_app_permissions` rows — see ADR-025).
> - **App (Lovable) — PENDING:** `/referrals` page + create + FR-REF-07 history; referral surfacing (LeadSource=Referral, introduced-by badge, outgoing list, **manual** outcome); Documents tab on `ContactDetail` (link-only over the CDL `document` resource); Referrals nav + `'referrals'` `role_configurations` page-key seed.

**Goal (re-scoped)**: a broker can create a `Referral` row that ties two `Contacts` and a `Member` together with a manual outcome, and attach link-only `Document` rows to a Contact — both over governed CDL resources. (The offer lifecycle + automatic referral close-the-loop move to the deferred offer workstream.)
