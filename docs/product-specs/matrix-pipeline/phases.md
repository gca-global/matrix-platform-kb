...
> Reflects the deployed `matrix-pipeline-2-0` app codebase as of **2026-06-08** (Week-4 transactions rework + referrals + documents, app `a040bd5`) and platform-side (Cursor) CDL surface through 2026-06-08 (Referrals/Documents + the transaction-phase `cdl-listing-lifecycle` path, ADR-025/ADR-026). Tags: **DONE** / **MOSTLY DONE** / **PARTIAL** / **BUILT-NOT-WIRED** / **NOT STARTED**. The per-week sections below carry the same status callout.
...
**Immediate next** — Weeks 1-4 are closed (both lanes). Week 4 shipped re-scoped: Referrals + Documents + the transaction phase (Pipeline-owned `Property` transaction phase via `cdl-listing-lifecycle`, ADR-026), platform (Cursor) + app (Lovable, app `a040bd5`) both done. **Next is Week 5 — Pipeline projection + Commission Engine** (PARTIAL): the `/pipeline` v0 board + Matching signal are live; still to build are the per-`(Contact × SavedSearch)` projection, the Commission Engine (cost events / rules / deal P&L / compensation), the forecast & variance dashboards, and `/reports`. **Still deferred (ADR-025):** in-flight offer *negotiation* economics, FR-REF-08 auto close-the-loop, forecast P&L on the TM card. The only deferred Week-2 item is the Prospecting **email channel** (stretch) — blocked on a platform mailer, tracked alongside the same gap in `sso-member-roster-lint`.
