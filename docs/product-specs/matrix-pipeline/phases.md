...
- (~2 h) **CDL EFs `cdl-showing-write`** (handles all five Showing-chain resources behind one router by `resource` query param). Scope: `pipeline:showings:write`. ES256 + scope check.
- (~1 h) **CDL EF `cdl-showing-read`**: paginated reads + estimated counts.
- (~2 h) **CDL EF `cdl-caravan-write`** + `cdl-caravan-read`: writes `Caravan` + `CaravanStop` in CDL (if migrated) or CRM app DB (if not — confirm via MCP first).
- (~1 h) Live-state refresh + verify `public.showings` exists with the column shape Lovable expects. Patch wiki if drift detected.
- (~1 h) `cdl-schema.md` drift PR (platform team): rename guidance for `showing_appointments` → `showings`.
- (Stretch ~2 h) `Showing` post-hoc record table (separate from `ShowingAppointment`): proposal note in `wiki/architecture.md#phase-2-migration`.
