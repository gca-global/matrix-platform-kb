...
> Reflects the deployed `matrix-pipeline-2-0` app codebase as of **2026-06-12** (Week-5 Pipeline projection + Commission Engine + `/reports`, `origin/main` @ `3255cea`; plus finance-ERP app EFs at `de1e7b0`) and the platform-side (Cursor) CDL/SSO surface through 2026-06-10 (RLS Pattern B on `contact_listings`/`contact_listing_notes` + ADR-028). Tags: **DONE** / **MOSTLY DONE** / **PARTIAL** / **BUILT-NOT-WIRED** / **NOT STARTED**. The per-week sections below carry the same status callout.
...
- **Week 4 — Transactions + Referral + Documents**: **DONE (re-scoped)** (2026-06-08). *Platform (Cursor):* Referrals + Documents (`public.referral`/`public.document` CDL resources + `cdl-read`/`cdl-write` v7 + ADR-025); **transaction phase (ADR-026)** — Pipeline owns the `Property` *transaction phase* via the governed **`cdl-listing-lifecycle`** EF (`acceptOffer`/`closeDeal`/`cancelDeal`) + canonical `Property` close economics (migration `20260608150000`) + buyer via `ContactListings` + `HistoryTransactional` timeline (`cdl-read` v8). *App (Lovable, app `a040bd5`):* `/transactions` reworked to the event-sourced model (`useTransactions.ts` — `useAcceptOffer`/`useCloseDeal`/`useCancelDeal` over `cdl-listing-lifecycle` + `useTransactionTimeline`; `Transactions.tsx` — `PropertyPicker`+`ContactPicker`, accept/close/cancel with Close/Cancel gated to global/org_admin/system_admin, `TransactionTimeline`); `/referrals` page + `ContactReferralsCard` (manual outcome) + referrals nav; `ContactDocumentsTab` (link-only over the `document` resource) on `ContactDetail`. Only in-flight offer *negotiation* economics + FR-REF-08 auto close-the-loop + forecast P&L stay deferred (ADR-025).
- **Week 5 — Pipeline + Commission**: **DONE** (2026-06-10, [ADR-028](../../architecture/decisions/ADR-028.md)). `/pipeline` reworked to the per-`(Contact × SavedSearch)` projection (`usePipeline` + filters + stage explainer); the **per-country, date-versioned Commission Engine** shipped (`commission_rule`/`commission_estimate`/`broker_compensation`/`deal_cost_event`/`cost_rate_card` + `commission:admin` action key) with the full Hungary P&L waterfall (`compute.ts` reproduces `FalkMiksa.xlsx` exactly: gross 10,800,000 / net 8,503,937 / gross profit 7,823,622 / PBT 5,085,354 = 47.1% / net profit 4,627,672 = 42.8%); the deal P&L tab + variance card; the `finance-erp-webhook`/`finance-erp-reconcile` app EFs (deployed to `kzvhqgpedapzqmwgikrw`); and `/reports/forecast` + `/reports/variance`. **Operator residual:** set `FINANCE_ERP_WEBHOOK_SECRET` + `CDL_INTERNAL_SECRET` and apply the `pg_cron` reconcile schedule.
- **Week 6 — AI Copilot**: **NOT STARTED** (deferred whole — out of scope by sequencing decision; `O-AI-COPILOT` stays planned).
- **Week 7 — Staging hardening**: **PARTIAL**. *Cursor lane DONE:* RLS Pattern B on `contact_listings` + `contact_listing_notes` (closes R2); docs/ADR closeout (ADR-028 finalized, wiki/roadmap/log refreshed). *Lovable lane + operator PENDING:* Playwright happy-path suite, staging seed data, polish + 60-min demo script; secrets/cron + lint two-day attestation + the demo-outcome `phase-checkpoint`.
...
**Immediate next** — Weeks 1-5 are closed. Week 4 shipped re-scoped: Referrals + Documents + the transaction phase (Pipeline-owned `Property` transaction phase via `cdl-listing-lifecycle`, ADR-026). **Week 5 — Pipeline projection + Commission Engine is DONE** (2026-06-10, [ADR-028](../../architecture/decisions/ADR-028.md)): per-`(Contact × SavedSearch)` projection, the per-country/date-versioned Commission Engine (HU waterfall reproduces `FalkMiksa.xlsx`), the deal P&L tab + variance card, the `finance-erp-webhook`/`finance-erp-reconcile` app EFs, and `/reports`. **Week 6 — AI Copilot is deferred whole** (out of scope by decision). **Next is Week 7 — Staging hardening:** the Cursor lane is already done (RLS Pattern B + docs/ADR closeout); remaining is the Lovable lane (Playwright happy-path suite, staging seed data, polish + 60-min demo script) + operator setup (`FINANCE_ERP_WEBHOOK_SECRET`/`CDL_INTERNAL_SECRET`, `pg_cron` reconcile, lint attestation, demo-outcome checkpoint). **Still deferred (ADR-025):** in-flight offer *negotiation* economics, FR-REF-08 auto close-the-loop, forecast P&L on the TM card. The only deferred Week-2 item is the Prospecting **email channel** (stretch) — blocked on a platform mailer, tracked alongside the same gap in `sso-member-roster-lint`.
...
> **Status (2026-06-08): DONE (re-scoped); transaction phase un-deferred (ADR-026).** Both lanes shipped. Pipeline owns the *transaction phase* of the shared `Property` (ownership is by process, not table — ADR-026), modeled event-sourced on canonical homes: buyer = a `Contact` via `ContactListings`, subject = `Property`, timeline = `HistoryTransactional`. **`transaction_management` stays the bare RESO 4-field header** (no link columns). **Still DEFERRED (ADR-025):** in-flight offer *negotiation* economics (amount/currency, counter/sign lifecycle), the FR-REF-08 automatic referral close-the-loop, and the forecast P&L block — kept clean of the datamodel (no `x_` offer columns).
>
> Week-4 delivers **Transaction phase (FR-TM, transaction-phase subset) + Referrals (FR-REF) + Documents (FR-DOC)**.
...
> - **App (Lovable) — DONE (2026-06-08, app `a040bd5`):** (Transactions) `useTransactions.ts` reworked — `useAcceptOffer`/`useCloseDeal`/`useCancelDeal` over `cdl-listing-lifecycle` + `useTransactionTimeline` over `cdl-read` `history_transactional`; legacy direct-history stopgaps removed; `Transactions.tsx` uses `PropertyPicker` (subject) + `ContactPicker` (buyer), accept/close/cancel actions (Close/Cancel gated to global/org_admin/system_admin), and a `TransactionTimeline`. (Referrals/Docs) `/referrals` page + `ContactReferralsCard` (manual outcome) + referrals nav; `ContactDocumentsTab` (link-only over the `document` resource, `useDocuments`) on `ContactDetail`.
...
- ✅ DONE (app `a040bd5`) Rework `/transactions` to the event-sourced model (ADR-026): `useTransactions.ts` — dropped `useRecordPropertyClose`/`useRecordBackOnMarket`; added `useAcceptOffer`/`useCloseDeal`/`useCancelDeal` (POST `cdl-listing-lifecycle`) + `useTransactionTimeline` (`cdl-read` `history_transactional`). `Transactions.tsx` — `PropertyPicker` (subject) + `ContactPicker` (buyer via `ContactListings`), accept/close/cancel actions (Close/Cancel gated to global/org_admin/system_admin), and the `TransactionTimeline`. Cite [transaction-lifecycle.md](../../business-processes/canonical-processes/processes/transaction-lifecycle.md) + [ADR-026](../../architecture/decisions/ADR-026.md).
- ✅ DONE `transaction_management` create/list/type stays as-is (`useCreateTransaction`/`useSetTransactionType` over `cdl-write`) — the bare RESO 4-field header; no link columns (ADR-026).
...
- ✅ DONE (app `a040bd5`) Documents tab on `ContactDetail` (`ContactDocumentsTab` + `useDocuments`): link-only over the CDL `document` resource (external storage URL, file not stored). Cite [wiki/requirements.md#fr-doc-documents](wiki/requirements.md#fr-doc-documents) (FR-DOC-01..05).
- ✅ DONE (app `a040bd5`) `/referrals` page + creation (referrer/referee `Contacts` + responsible `Member` + type) + `ContactReferralsCard` with **manual** outcome + referrals nav. Cite [wiki/requirements.md#fr-ref-referral](wiki/requirements.md#fr-ref-referral) (FR-REF-01..03).
...
> **Status (2026-06-10): DONE ([ADR-028](../../architecture/decisions/ADR-028.md)).** All functional Week-5 scope shipped and verified against the live app DB + committed code (`origin/main` @ `3255cea`). `/pipeline` reworked to the **per-`(Contact × SavedSearch)` projection** (`usePipeline.ts` — one card per Prospecting row + a fallback no-saved-search card, stage still a pure projection from `pipelineProjection.ts`, with broker/team/OUID/country/budget/age filters + the stage explainer). The **Commission Engine** shipped as a per-country, date-versioned, app-private customization (`commission_rule`/`commission_estimate`/`broker_compensation`/`deal_cost_event`/`cost_rate_card` + Pattern B RLS + the `commission:admin` action key) with the full **Hungary P&L waterfall** (`src/lib/commission/compute.ts` reproduces `Deal_Hungary_PL FalkMiksa.xlsx` exactly: gross 10,800,000 / net 8,503,937 / gross profit 7,823,622 / PBT 5,085,354 = 47.1% / net profit 4,627,672 = 42.8%; CY/KZ seeded as drafts). The deal P&L tab (`DealPnLTab` + `WaterfallTable` + `DealNarrativeHeader` + `DealCostLogCard` + `BrokerCompensationCard` + `CommissionRuleAdminPanel`, dual-currency, inline overrides), the **variance card** (`VarianceCard` reads server-populated `actual_gci`/`variance`), the **`finance-erp-webhook` + `finance-erp-reconcile`** app EFs (deployed to `kzvhqgpedapzqmwgikrw`, committed `de1e7b0`), and **`/reports/forecast` + `/reports/variance`** (`useCommissionReports`, `processCoverage` all live) are all built. See the AS-BUILT block below for the plan→shipped deltas. **Operator residual only:** set `FINANCE_ERP_WEBHOOK_SECRET` + `CDL_INTERNAL_SECRET` and apply the `pg_cron` reconcile schedule.
