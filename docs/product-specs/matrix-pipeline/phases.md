...
- (~1 h) Add i18n scaffold (EN + RU) — load EN strings only for now. Cite the platform `app-template.md`. **Delivered 2026-06-02 at full scope**: EN + RU + **HU** complete locales, plus a tenant-admin terminology layer (Settings → "Translations & Labels") where UI nouns are resolved from the CDL corpus and can be renamed/translated per locale without code changes. See [ADR-020](../../architecture/decisions/ADR-020.md) + [docs/data-models/reso-dd-descriptions.md](../../data-models/reso-dd-descriptions.md).
