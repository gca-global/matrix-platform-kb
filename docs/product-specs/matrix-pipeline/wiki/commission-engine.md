...
|---|---|---|
| `deal_cost_event` | `DealCostEvent` | A single attributed operational cost; `category` column (type `cost_category` enum: legal / marketing / conference / showing / negotiation / client_care / research / other + the Prompt-2b additions notary / translation / postal / internal_referral / internal_marketing_legal); manual or auto-derived (`source` enum) from a tagged `Activity` (`activity_id`, FR-ACT-10); keyed by `listing_key` (+ optional `transaction_key`/`member_key`) |
...
| `commission_estimate` | `DealPnL` (generalized) | Per **listing/sales-contract** estimate (subject = `listing_key`; optional `transaction_key`/`contact_key`): resolved `country_code` + `commission_rule_id`, `base_amount`/`base_source`/`currency_code`. **Materialized waterfall columns**: `gross_commission`, `statutory_deductions`, `net_commission`, `attributed_costs`, `net_margin`, `gross_profit`, `profit_before_tax`, `net_profit` (+ `actual_gci`/`variance`, written server-side on reconciliation). The **full step-by-step breakdown** (referral, royalty, broker fees, corporate tax, …) lives in **`computation jsonb`**; dual currency via `fx_rate`/`display_currency`; deal narrative in `deal_notes` (+ `deal_date`/`proceeds_entity`/`object_type`/`payer`); per-deal tweaks in `overrides jsonb` |
...
1. CRM forecasts GCI + broker compensation at each funnel stage; stores results in app-private tables (`commission_estimate`, `broker_compensation`).
