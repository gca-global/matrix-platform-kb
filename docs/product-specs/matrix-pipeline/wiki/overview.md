...
The CRM data model has **no** stored `Opportunity` entity and no separate pipeline-stage table. The 5-stage pipeline exists only as a **UI/UX projection** of canonical RESO state. Each client intent (what other CRMs would call an "opportunity") is represented by a pair `(Contacts × SavedSearch)` + the related `Prospecting` / `ContactListings` / Showing chain rows / `TransactionManagement` / the target `Property.StandardStatus`.

One Contact can have **multiple parallel `SavedSearch` rows** — these are the "multiple parallel intents" (residence + investment + relocation), each with its own projected stage.
...
- **Explicit project-flavour exception 1: Deal Commercialization, GCI, and Commission Engine** ([wiki/commission-engine.md](commission-engine.md)). Internal ERP-lite with app-private entities (`DealCostEvent`, `CostRateCard`, `CommissionRule`, computed views `DealPnL` / `BrokerCompensation`) is required for sales-broker commission transparency; canonical RESO does not cover this use case (see transaction-lifecycle Non-goals), `matrix-fm` remains entity-level. Reconciliation with external Finance ERP — see [wiki/commission-engine.md#reconciliation](commission-engine.md#reconciliation). Rationale — see [wiki/architecture.md#escape-hatch](architecture.md#escape-hatch).
