...
| Broker Dashboard page key not in default role permissions | Admin must explicitly enable the `broker-dashboard` page permission in Meeting Hub if access to the broker-specific dashboard is needed (this is the Meeting Hub permission key per `src/hooks/useAppPermissions.ts`, distinct from the `pipeline` permission used by Matrix Pipeline) | Configuration |
